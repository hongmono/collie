...
export function updatesNewerThan(tags: string[], current: string): string[] {
  ...
  const downstream = isDownstreamRevisionVersion(current);
  ...
  for (const tag of tags) {
    ...
    if (downstream) {
      if (!DOWNSTREAM_REVISION.test(parsed.prerelease ?? "")) continue;
    } else if (parsed.prerelease !== null && !train) continue;
