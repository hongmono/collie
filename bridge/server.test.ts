...
describe("the host gate — `?host=` selects among enrolled members and nothing else", () => {
  ...
  test("every session-scoped route resolves through the gate, never past it", () => {
    ...
    //
    // All ELEVEN session-scoped routes (artifact list/content, tab create, workspace create, launch,
    // this host's launcher rows, tab action, the pane family, "look now", the worktree listing and
