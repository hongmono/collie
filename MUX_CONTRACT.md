...
| `resizeGrid` | `POST /api/pane/:id/resize` when a writable device opens the pane | `HERDR_SOCKET_PATH=… herdr terminal session control <pane> --cols … --rows …` without `--takeover`, then immediate NDJSON release (**API** § One-shot PTY resize; ADR 0035) | **no** — not probed or implemented | **no** — not probed or implemented |
